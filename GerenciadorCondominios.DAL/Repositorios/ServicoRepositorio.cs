...
namespace GerenciadorCondominios.DAL.Repositorios
{
    public class ServicoRepositorio : RepositorioGenerico<Servico>, IServicoRepositorio
    {
        ...
        public async Task<IEnumerable<Servico>> recuperarPorUsuario(string codigoUsuario)
        {
            try
            {
                return await _contexto.Servicos.Where(x => x.CodigoUsuario == codigoUsuario).ToListAsync();
            }
            catch (Exception ex)
            {
                throw ex;
